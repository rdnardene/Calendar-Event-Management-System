...
function doLogin()
{
	...
	if ($row['status'] == 'lock') {
		echo"<script>alert('This Account is Locked!')</script>";
	} else {
		if (dbNumRows($result) == 1 && $row['status'] == 'active') {
			...
			echo"<script>alert('This Account is Inactive!')</script>";
		}
	}
...
function getBookingRecords(){
	...
	$sql 	= "SELECT u.id AS uid, u.name, r.department, u.address,
			   r.ucount, r.rdate, r.Event_Name, r.status, r.comments   
